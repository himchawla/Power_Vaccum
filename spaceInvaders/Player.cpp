...
void player::Update(float _dT)
{
	...
	transform.m_Accelaration += (transform.m_Force / transform.m_Mass) * _dT * 150.0f;
	...
	if (Magnitude(transform.m_Accelaration) > 0.0f)
	{
		...
		{
			transform.m_Accelaration.x -= transform.m_Friction.x * (abs(transform.m_Accelaration.x) / transform.m_Accelaration.x) * _dT * 100.0f;
		}
		else
		...
		{
			transform.m_Accelaration.y -= transform.m_Friction.y * (abs(transform.m_Accelaration.y) / transform.m_Accelaration.y) * _dT * 100.0f;
		}
		else
